...
If you manage the client lifecycle yourself:
...
def useExistingClient(client: ValkeyClient) =
  Valkey[IO].fromClient[String, String](client)
```
...
In order to create a client and connection you must provide a `Log` instance. You can use one of the built-in instances or provide your own.
...
// No-op logger (available for any Applicative[F])
...
// Simple STDOUT logger (available for any Sync[F])
given Log[IO] = Log.Stdout.instance[IO]
```

### log4cats integration

For structured logging, use the `valkey4cats-log4cats` module which derives a `Log[F]` from a `Logger[F]`:
