...
Each variant carries the full `message: String` from the server.
...
// Discard error info, get Option
val opt: Option[String] = response.toOption  // Some("hello")

// Preserve error as Left
val either: Either[_, String] = response.toEither  // Right("hello")
...
Valkey[IO].utf8("valkey://localhost:6379").use { valkey =>
  for
    // liftTo raises ValkeyDomainError on Err, extracts value on Ok
...
The raised exception is `ValkeyResponse.ValkeyDomainError`, which you can pattern match on:

```scala mdoc:compile-only
import cats.effect.*
import dev.profunktor.valkey4cats.model.ValkeyResponse.ValkeyDomainError

val program: IO[Unit] = IO.raiseError(new RuntimeException("test"))

program.handleErrorWith:
  case ValkeyDomainError(error) =>
    IO.println(s"Valkey domain error: ${error.message}")
  case other =>
    IO.println(s"Other error: ${other.getMessage}")
```

## Cats type class instances

`ValkeyResponse` has instances for:

- `Functor` -- transform the success value with `map`
- `Monad` -- sequence dependent operations with `flatMap`
- `MonadError[ValkeyResponse, ValkeyError]` -- raise/handle errors within ValkeyResponse
- `Traverse` -- sequence effects inside ValkeyResponse

This means you can use standard Cats combinators:
...
This means your application logic can handle domain errors explicitly (pattern match, fold, recover) without `try/catch` style exception handling, while truly exceptional infrastructure failures remain in the effect type where they belong.
