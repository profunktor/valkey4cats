...
Valkey[IO].utf8("valkey://localhost:6379").use { valkey =>
  for
    ...
    _ <- IO.println(s"TTL: ${ttl.toOption}") // Some(60) approximately

    // PEXPIRE and PTTL (millisecond precision)
    _ <- valkey.pexpire("ephemeral", 30000)
    pttl <- valkey.pttl("ephemeral")
    _ <- IO.println(s"PTTL: ${pttl.toOption}") // Some(~30000)

    // PERSIST (remove expiration)
    _ <- valkey.persist("ephemeral")
    ttlAfter <- valkey.ttl("ephemeral")
    _ <- IO.println(s"TTL after persist: ${ttlAfter.toOption}") // Some(-1)
...
| EXPIRETIME | `expireTime(key)` | `F[ValkeyResponse[Long]]` |
| PEXPIRETIME | `pexpireTime(key)` | `F[ValkeyResponse[Long]]` |
...
| OBJECT ENCODING | `objectEncoding(key)` | `F[ValkeyResponse[Option[String]]]` |
...
| SORT STORE | `sortStore(key, dest)` | `F[ValkeyResponse[Long]]` |
| DUMP | `dump(key)` | `F[ValkeyResponse[Option[Array[Byte]]]]` |
| RESTORE | `restore(key, ttl, bytes)` | `F[ValkeyResponse[Unit]]` |
| WAIT | `waitReplicas(numReplicas, timeout)` | `F[ValkeyResponse[Long]]` |
| MOVE | `move(key, db)` | `F[ValkeyResponse[Boolean]]` |
