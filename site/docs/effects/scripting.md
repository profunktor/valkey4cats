...
Valkey4Cats supports server-side function execution via the `FCALL` command (Valkey Functions), as well as script cache management commands.
...
Valkey[IO].utf8("valkey://localhost:6379").use { valkey =>
  for
    ...
    // (Assumes a function "myfunc" has been loaded via FUNCTION LOAD)
