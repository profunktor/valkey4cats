...
Valkey[IO].utf8("valkey://localhost:6379").use { valkey =>
  for
    ...
    slen <- valkey.strlen("greeting")
    _ <- IO.println(s"STRLEN: ${slen.toOption}")              // Some(13)

    // GETRANGE / SETRANGE
    sub <- valkey.getRange("greeting", 0, 4)
    _ <- IO.println(s"GETRANGE 0..4: ${sub.toOption}") // Some("Hello")
