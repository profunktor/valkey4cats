...
import dev.profunktor.valkey4cats.results.{ScanResult, ScoredValue}
...
trait SortedSetCommands[F[_], K, V] {
  ...
  def zrangeWithScores(
      ...
      start: Long,
      stop: Long
  ): F[ValkeyResponse[List[ScoredValue[V]]]]
  ...
  /** Remove and return the member with the lowest score from a sorted set
    ...
    */
  def zpopmin(key: K): F[ValkeyResponse[Option[ScoredValue[V]]]]
  ...
  /** Remove and return up to count members with the lowest scores from a sorted set
    ...
    */
  def zpopminCount(key: K, count: Long): F[ValkeyResponse[List[ScoredValue[V]]]]
  ...
  /** Remove and return the member with the highest score from a sorted set
    ...
    * @return The removed (member, score) pair, or None if the sorted set is empty
    */
  def zpopmax(key: K): F[ValkeyResponse[Option[ScoredValue[V]]]]
  ...
  /** Remove and return up to count members with the highest scores from a sorted set
    ...
    * @return List of removed (member, score) pairs
    */
  def zpopmaxCount(key: K, count: Long): F[ValkeyResponse[List[ScoredValue[V]]]]
  ...
  /** Return the difference between the first sorted set and all successive sorted sets, with scores.
    ...
    */
  def zdiffWithScores(keys: K*): F[ValkeyResponse[List[ScoredValue[V]]]]
  ...
  /** Return the union of all given sorted sets, with scores.
    ...
    */
  def zunionWithScores(keys: K*): F[ValkeyResponse[List[ScoredValue[V]]]]
  ...
  /** Return the intersection of all given sorted sets, with scores.
    ...
    */
  def zinterWithScores(keys: K*): F[ValkeyResponse[List[ScoredValue[V]]]]
  ...
  def zmpop(
      keys: List[K],
      filter: ScoreFilter
  ): F[ValkeyResponse[Option[(K, List[ScoredValue[V]])]]]
  ...
  def zscan(
      key: K,
      cursor: String
  ): F[ValkeyResponse[ScanResult[List[ScoredValue[V]]]]]
}
