...
function prepareIdxAndDocMap() {
  const docs = [  
    ...
    } ,    
    {
      "title": "Effects API",
      "url": "/effects/",
      "content": "Effects API The API that operates at the effect level F[_] on top of cats-effect. Bitmaps API Connection API Geo API Hashes API Keys API Lists API Scripting API Server API Sets API Sorted Sets API Strings API"
    } ,    
    {
      "title": "Streams API",
      "url": "/streams/",
    ...
    } ,    
    {
      "title": "Home",
      "url": "/",
      "content": "valkey4cats Valkey client built on top of Cats Effect, Fs2 and the async Java client Lettuce. ⚠️ Valkey fork is currently being updated ⚠️ Quick Start import cats.effect.* import cats.implicits.* import dev.profunktor.valkey4cats.Redis import dev.profunktor.valkey4cats.effect.Log.Stdout.given object QuickStart extends IOApp.Simple: val run: IO[Unit] = Redis[IO].utf8(\"redis://localhost\").use: redis =&gt; for _ &lt;- redis.set(\"foo\", \"123\") x &lt;- redis.get(\"foo\") _ &lt;- redis.setNx(\"foo\", \"should not happen\") y &lt;- redis.get(\"foo\") _ &lt;- IO(assert(x === y)) yield () If you like it, give it a ⭐ ! If you think we could do better, please let us know! Versions The 1.x.x series is built on Cats Effect 3 whereas the 0.x.x series is built on Cats Effect 2. Dependencies Add this to your build.sbt for the Effects API (depends on cats-effect): libraryDependencies += \"dev.profunktor\" %% \"valkey4cats-effects\" % Version Add this for the Streams API (depends on fs2 and cats-effect): libraryDependencies += \"dev.profunktor\" %% \"valkey4cats-streams\" % Version Log4cats support valkey4cats needs a logger for internal use and provides instances for log4cats. It is the recommended logging library: libraryDependencies += \"dev.profunktor\" %% \"valkey4cats-log4cats\" % Version Running the tests locally Start both a single Redis node and a cluster using docker-compose: &gt; docker-compose up &gt; sbt +test If you are trying to run cluster mode tests on macOS you might receive host not found errors. As a workaround add new environment variable in docker-compose.yml for RedisCluster: IP=0.0.0.0 The environment section should look like this: environment: - INITIAL_PORT=30001 - DEBUG=false - IP=0.0.0.0 Code of Conduct See the Code of Conduct. Contribution Guidelines Going forward , features must be compatible with the Valkey OSS project. LICENSE Licensed under the Apache License, Version 2.0 (the “License”); you may not use this project except in compliance with the License. You may obtain a copy of the License at http://www.apache.org/licenses/LICENSE-2.0. Unless required by applicable law or agreed to in writing, software distributed under the License is distributed on an “AS IS” BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied. See the License for the specific language governing permissions and limitations under the License."
    } ,    
    {
      "title": "Keys",
